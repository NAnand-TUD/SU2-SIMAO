...
//     virtual unsigned long getnModes(void);
//   
//     virtual su2double getModeVector(unsigned short iMode, unsigned short val_iDim, unsigned long iPoint);
//   
//     virtual su2double getFrequency(unsigned short iMode);
//     
//     virtual unsigned long getnCSD_Points(void);
// 
//     virtual su2double getRefLength(void);
};
...
class CPhysicalGeometry : public CGeometry {
  ...
  unsigned long **cellConnectivity; /*!< \brief CSD cell connectivity list. */
  su2double* omega;                 /*! <\brief modal reduced frequencies. */
  su2double refLength;              /*! <\brief Reference length to scale CSD model. */
  vector<su2double> modeShapes;
  vector<su2double> CSD_Coords;
  unsigned long nModes,         /*!< \brief n. of modes for CSD model. */
  nCSDPoints,                   /*!< \brief n. of points on CSD mesh. */
  nLocal_CSD_Elem,
  nCSD_Elem,
  nCSD_Tria,
  nCSD_Quad,
...
public:
  ...
  // reads CSD points and mode shapes
...
/*!
 * \class CModalGeometry
 * \brief Class for mapping CSD mesh deformation to CFD surface mesh 
 * and fluid forces to structural model
 * \author S. Marques
 */
// class CModalGeometry : public CGeometry {
//     unsigned long **cellConnectivity; /*!< \brief CSD cell connectivity list. */
//     su2double* omega;                 /*! <\brief modal reduced frequencies. */
//     su2double refLength;              /*! <\brief Reference length to scale CSD model. */
//     vector<su2double> modeShapes;
 ...
 * \author S. Marques
 */
class CRBFMapping : public CGeometry {

    su2double *phi;			    /*!< \brief Radial Basis Function values  */
//     su2double **Aa,**As;        /*!< \brief RBF interpolation matrix blocks */
//     su2double **Ha;             /*!< \brief RBF mapping matrix */
//     vector<double> XV;          /*!< \brief FE input nodes  */
    char rbfFunctionType[200];
...
// 	 * \param[in] val_nZone - Total number of domains in the grid file.
// 	 */
// 	CRBFMapping(CConfig *config, unsigned short val_iZone, unsigned short val_nZone);
  
    /*!
    * \overload
    * \brief 
    * \param[in] geometry - Definition of the geometry container holding the 
    * initial linear partitions of the grid + coloring.
    * \param[in] config - Definition of the particular problem.
    */
    CRBFMapping(CGeometry *geometry, CConfig *config);
...
//    * \param[in] config - Definition of the particular problem.
//    */
//     CRBFMapping(CConfig *config);

    /*!
	 * \brief returns frequency associated with mode iMode.
     * \param[in] iMode - Select mode.
     * \param[out] value - Selected frequency value.
	 */
    su2double getWedlandC2(su2double radius);
    /*!
	 * \brief Destructor of the class.
	 */
	~CRBFMapping(void);
