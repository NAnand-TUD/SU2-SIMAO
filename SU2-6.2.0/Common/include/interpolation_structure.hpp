 ...
 * \param[in] geometry - Geometrical definition of the problem.
 * \param[in] config - Definition of the particular problem.
 * \param[in] iZone - index of the fluid zone -- for now, we're solving aeroelastic problems
 * using only fluid zone
 */
  CInterpolator(CGeometry ****geometry_container, CConfig **config, unsigned int iZone);
...
public:
   ...
   * \param[in] iZone - index of the donor zone
   */
  CRadialBasisFunction(CGeometry ****geometry_container, CConfig **config);
