...
CPhysicalGeometry::CPhysicalGeometry(CConfig *config, unsigned short val_iZone, unsigned short val_nZone) : CGeometry() {
  ...
  /*--- CSD arrays & vectors ---*/
    ...
    // read CSD Mesh
//     if(config->GetKind_Solver() == FEM_MODAL) ReadCSD_Mesh(config);
//     else if(config->GetKind_GridMovement(ZONE_0) == AEROELASTIC) ReadCSD_Mesh(config);
//     ReadCSD_Mesh(config);   //WARNING: remove call -- only for testing
            ...
            for (iElem_Bound = 0; iElem_Bound < nElem_Bound[iMarker]; iElem_Bound++) {
              ...
              switch(VTK_Type) {
                case LINE:
                  ...
                  if (nDim == 3 && config->GetKind_Solver() != 14){
                      //WARNING: config->GetKind_Solver() != 14 allows use of 2D shell models for CSD solver
                      cout << multizone_file << " mesh file=> " << cstr << config->GetKind_Solver() << endl;
                    SU2_MPI::Error("10724-Please remove line boundary conditions from the mesh file!", CURRENT_FUNCTION);
                  }
...
// CModalGeometry::CModalGeometry() : CGeometry() {
//   
//   size = SU2_MPI::GetSize();
//   rank = SU2_MPI::GetRank();  
// 
...
// CModalGeometry::CModalGeometry(CConfig *config) : CGeometry() {
//     /*--- CSD arrays & vectors ---*/
//     modeShapes.clear();
//     CSD_Coords.clear();
// };

void CPhysicalGeometry::ReadCSD_Mesh(CConfig *config){
    //TODO: modes and fe mesh to be stored in geometry->csdnodes (same type of var as cfd "node")
    //      add options to config file (hardwire test case for now?!)
    //
    unsigned long ipoint,iMode;
	unsigned short number_of_modes;
//     unsigned long *cellNodes;
	vector<unsigned long>::iterator it;
...
//     rank = SU2_MPI::GetRank();  
    ...
    Uinf = 1.;  //TODO: get freestream value and do it properly!!!
    
    /* --- Read in CSD Mesh point coordinates and connectivity --- */
    
    //using simplified version of FE mesh 
//     strcpy(cstr,"csdMesh.su2");
//     
//     // strcpy(cstr, val_modes_filename.c_str());
// 	mesh_file.open(cstr, ios::in);
//     
...
//     iCount = 0;
//     while (getline (mesh_file, text_line)) {
//         position = text_line.find ("POINTS",0);
//         if (position != string::npos) {
//             text_line.erase (0,6); 
//             nCSDPoints = atoi(text_line.c_str());
//             cout << "\nnumber of CSD mesh nodes: " << nCSDPoints << endl;
//             iCount++;
//             XP      = new su2double[nDim*nCSDPoints];
//             CSD_Coords.reserve(nDim*nCSDPoints);
//         
//             //read all points
//             for(ipoint = 0; ipoint < nCSDPoints; ++ipoint) {
//                 getline(mesh_file, text_line);
//                 istringstream point_line(text_line);
//                 point_line >> Coord_3D[0]; point_line >> Coord_3D[1]; point_line >> Coord_3D[2];
//                 XP[0*nCSDPoints + ipoint] = Coord_3D[0];
//                 XP[1*nCSDPoints + ipoint] = Coord_3D[1];
//                 XP[2*nCSDPoints + ipoint] = Coord_3D[2];
//             }
//              
//             for( iDim = 0; iDim < nDim; ++iDim){
//                 for (ipoint = 0 ; ipoint < nCSDPoints; ipoint++) {
//                     CSD_Coords.push_back(XP[iDim*nCSDPoints + ipoint]);
//                 }
//             }
//             
//         }
...
//     mesh_file.close();
//        
//     /*--- Read the information about CSD elements ---*/
//   	mesh_file.open(cstr, ios::in);
// 
//     while (getline (mesh_file, text_line)) {
//         position = text_line.find ("NELEM=",0);
...
//             text_line.erase (0,6);
//             stringstream stream_line(text_line);
//             stream_line >> nCSD_Elem;         //Global_nElem;
// 
//             cout << nCSD_Elem << "\telements in CSD model before parallel partitioning." << endl;
// 
//             /*--- Allocate space for elements ---*/
//             cellConnectivity = new unsigned long *[nCSD_Elem];
//             for (iElem = 0; iElem < nCSD_Elem; ++iElem) cellConnectivity[iElem] = NULL;
// 
...
//                     grid).                                                          ---*/
//             for (iElem = 0; iElem < nCSD_Elem; ++iElem) {
// 
//                 getline(mesh_file, text_line);
//                 istringstream elem_line(text_line);
//                 /*--- Read the value that defines the element type and
//                 *nodes that make up the element.   ---*/
//                 elem_line >> np;
//                 cout << "elem line: " << np << endl;
//                 cellConnectivity[iElem]     = new unsigned long[np+1];      //allocate space to store n. of cell points
//                 cellConnectivity[iElem][0] = np;
//                 for( ipoint = 0; ipoint < np; ++ipoint) elem_line >> cellConnectivity[iElem][ipoint+1] ;
//             }
//         }
//     }
	...
	modeShapes.reserve(nDim*nCSDPoints*nModes);
    omega = new su2double [nModes];
    XV = new su2double [nCSDPoints];
    YV = new su2double [nCSDPoints];
    ZV = new su2double [nCSDPoints];
...
//     for (iMode=0; iMode < nModes; ++iMode) generalizedVelocity[iMode] = new su2double [3];
	...
	for (iMode=0; iMode < nModes; ++iMode){
		...
		for (ipoint = 0 ; ipoint < nCSDPoints; ipoint++) modeShapes.push_back(XV[ipoint]);
        for (ipoint = 0 ; ipoint < nCSDPoints; ipoint++) modeShapes.push_back(YV[ipoint]);
        for (ipoint = 0 ; ipoint < nCSDPoints; ipoint++) modeShapes.push_back(ZV[ipoint]);
	}
...
// su2double CPhysicalGeometry::getFrequency(unsigned short iMode) { return omega[iMode]; };
// 
// unsigned long CPhysicalGeometry::getnModes(void) { return nModes; };
// 
// su2double CPhysicalGeometry::getModeVector(unsigned short iMode, unsigned short val_iDim, unsigned long iPoint) { 
//     return modeShapes[(iPoint + val_iDim*nPoint) + (iMode*nDim*nPoint)]; 
// };
// // 
// unsigned long CPhysicalGeometry::getnCSD_Points(void) { return nCSDPoints;};
// 
// su2double CPhysicalGeometry::getRefLength(void) { return refLength;};

// CRBFMapping::CRBFMapping() : CGeometry() {
//   
//     phi = NULL;    
// };
// 
// CRBFMapping::CRBFMapping(CGeometry *geometry, CConfig *config): CGeometry() {
//     
//     phi = NULL;
//     // compute matrices
// //     rbfSMatrix(geometry);
...
// }
// 
// CRBFMapping::~CRBFMapping(void) {
//   
//     delete [] phi;
// };
// 
// su2double CRBFMapping::getWedlandC2(su2double radius) {
//     return (1. - radius)*(1. - radius)*(1. - radius)*(1. - radius)*(4*radius+1);
// };
