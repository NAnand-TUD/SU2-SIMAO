...
public:
  ...
  virtual su2double GetModeVector(unsigned short val_mode, unsigned short val_dim);
  
  virtual void SetModeVector(unsigned short val_mode, unsigned short val_dim, su2double val);
  
  virtual void Add_DeltaVelSolution(unsigned short val_var, su2double val_solution);
};
...
/*!
 * \class CModalVariable
 * \brief Main class for defining the variables of the CSD Aeroelastic/Modal structural problem.
 * \ingroup Structural Finite Element Analysis Variables
 * \author F. Palacios, R. Sanchez, S. Marques.
 * \version 6.2.0 "Falcon"
 */
class CModalVariable : public CVariable {
protected:
    su2double *FlowTraction,        /*!< \brief Traction from the fluid field. */
    *FlowTraction_time_n;      /*!< \brief Traction from the fluid field at time n. */
  
    su2double *modeShape,           /*!< \brief mode shape vectors at CSD node */
    *ModalSolution,                 /*!< \brief solution at current time */
    *ModalSolution_t_n1,
    *ModalSolution_t_n2,
    *Solution_Vel,
    *Solution_Vel_time_n,
...
public:
   ...
   * \param[in] val_nDim - Number of dimensions of the problem.
   * \param[in] val_nvar - Number of variables of the problem.
   * \param[in] val_nModes - Number of structural modes
   * \param[in] config - Definition of the particular problem.
   */
  CModalVariable(su2double *val_csd, unsigned short val_nDim, unsigned short val_nvar, unsigned short val_nModes, CConfig *config);
  ...
  /*!
   * \brief Get the mode shape vector at structural nodes.
   * \param[in] val_mode - mode number;
   * \param[in] val_dim - vector component;
   * \return mode shape value/pointer.
   */
  su2double GetModeVector(unsigned short val_mode, unsigned short val_dim);
  
  void SetModeVector(unsigned short val_mode, unsigned short val_dim, su2double val);
  
  /*!
   * \brief Add to existing velocity solution value.
   * \param[in] val_var - velocity component;
   * \param[in] val_solution - value to add;
   * \return mode shape value/pointer.
   */  
  void Add_DeltaVelSolution(unsigned short val_var, su2double val_solution);

  inline void SetHarmonicBalance_Source(unsigned short val_var, su2double val_source);

  inline su2double GetHarmonicBalance_Source(unsigned short val_var);
};
