    ...
    switch (config[val_iZone]->GetKind_Solver()) {
      ...
      case DISC_ADJ_EULER:          case DISC_ADJ_NAVIER_STOKES:          case DISC_ADJ_RANS:
        ...
        if (direct_diff != NO_DERIVATIVE) {
          ...
          // testing -- start  
//           cout << "\n\ntest 1\n" << endl;
//           D_rho_test = solver_container[val_iZone][val_iInst][FinestMesh][FLOW_SOL]->node[239]->GetDensity();
//           cout << "\n\n rho[239]= " << D_rho_test;
//           D_rho_test = SU2_TYPE::GetDerivative(D_rho_test);
//           cout << "\t\t drho/dp = " << D_rho_test << endl;
          // testing -- end
      ...
      if (Wrt_Srf) {
        ...
        switch (FileFormat) {
          ...
          case PARAVIEW:
            ...
            if (rank == MASTER_NODE) cout << "8449 - Writing Paraview ASCII surface solution file." << endl;
...
void COutput::SetResult_Files_Parallel(CSolver *****solver_container,
  ...
  for (iZone = 0; iZone < val_nZone; iZone++) {
    ...
    /*--- This switch statement will become a call to a virtual function
     ...
     the local data for that particular problem alone. ---*/
      ...
      if (rank == MASTER_NODE)
        ...
        cout << "solver for instance: " << iInst << "; solver: " << config[iZone]->GetKind_Solver() << endl;
      switch (config[iZone]->GetKind_Solver()) {
      case EULER : case NAVIER_STOKES: case RANS :
          cout << "loading local data: euler\n";
        if (compressible)
    ...
    cout << " Write a slice on a structured mesh if requested. \n";
  ...
  cout << "complete writing binary restart files\n";
//   exit(0);
}
...
void COutput::LoadLocalData_Flow(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  /*--- Conserved Variables derivatives from Direct Differentiation ---*/
    if (direct_diff != NO_DERIVATIVE){
        nVar_Par += nVar_Consv_Par;
        
        Variable_Names.push_back("D_Density");
        Variable_Names.push_back("D_Momentum_x");
        Variable_Names.push_back("D_Momentum_y");
        if (geometry->GetnDim() == 3) Variable_Names.push_back("D_Momentum_z");
        Variable_Names.push_back("D_Energy");
    }
    // end test
  ...
  for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
    ...
    if (direct_diff != NO_DERIVATIVE){
        for (jVar = 0; jVar < nVar_First; jVar++) {
            D_var = solver[FirstIndex]->node[iPoint]->GetSolution(jVar);
            Local_Data[jPoint][iVar] = SU2_TYPE::GetDerivative(D_var);
            iVar++;
        }
    }
    // end test
...
void COutput::SortOutputData(CConfig *config, CGeometry *geometry) {
  ...
  unsigned long *nPoint_Linear = NULL;
...
//   nPoint_Linear = new unsigned long[size+1];
  ...
  for (int ii = 0; ii < size; ii++) {
    npoint_procs[ii] = nTotalPoint/size;
    cout << npoint_procs[ii] << endl;
    total_pt_accounted = total_pt_accounted + npoint_procs[ii];
    cout << total_pt_accounted << endl;
  }
...
void COutput::WriteRestart_Parallel_ASCII(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone, unsigned short val_iInst) {
  ...
  /*
     * =================================================================== 
     * Open new file with direct diff output straight to paraview format
     * to be appended to paraview flow output file
     * TEMPORARY SOLUTION
     * ===================================================================
     */
  
//     su2double D_rho_test;
//     cout << "\ntest 1\n" << endl;
//         
//     if (rank == MASTER_NODE) {
//         filename=string("direct_diff_flow.vtk");
//         restart_file.open(filename.c_str(), ios::out);
//         restart_file.precision(15);
//         restart_file.close();
//     }
...
//           
//                     /*--- Loop over the variables and write the values to file ---*/
//                     D_rho_test = solver[FLOW_SOL]->node[iPoint]->GetDensity();
//                     D_rho_test = SU2_TYPE::GetDerivative(D_rho_test);
//                     cout << iPoint << "\t drho/dp = " << D_rho_test << endl;
//                     restart_file << scientific << D_rho_test << "\t";
//                     
//                 }
//             }
//         }
...
void COutput::WriteRestart_Parallel_Binary(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone, unsigned short val_iInst) {
  ...
  unsigned long iPoint, iExtIter = config->GetExtIter();
  bool fem       = (config->GetKind_Solver() == FEM_ELASTICITY);
  bool modal     = (config->GetKind_Solver() == FEM_MODAL);
  bool adjoint   = (config->GetContinuous_Adjoint() ||
                    config->GetDiscrete_Adjoint());
  bool dual_time = ((config->GetUnsteady_Simulation() == DT_STEPPING_1ST) ||
                    (config->GetUnsteady_Simulation() == DT_STEPPING_2ND));
