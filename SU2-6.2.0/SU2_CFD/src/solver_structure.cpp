...
// functions to solve modal equations of motion.
// =============================================
// void CSolver::Aeroelastic3D(CSurfaceMovement *surface_movement, CGeometry *geometry, CConfig *config) {
// 	unsigned long iMode,nModes;
// // 	vector<unsigned long>::iterator it;
// // 	double refLength, frequency;
// // 	double Coord_3D[3];
// 	ifstream mode_file;
// // 	char cstr[200];
// 	string line,dummy;
...
// 
// 	// allocate and initialize modal coefficients
//     generalizedDisplacement = new su2double* [nModes];
//     generalizedVelocity     = new su2double* [nModes];
//     for (iMode=0; iMode < nModes; ++iMode){
// 		generalizedDisplacement[iMode]        = new su2double [3];
// 		generalizedVelocity[iMode]            = new su2double [3];
//         generalizedDisplacement[iMode][0]     = 0.0;
//         generalizedDisplacement[iMode][1]     = 0.0;
//         generalizedDisplacement[iMode][2]     = 0.0;
// 		generalizedVelocity[iMode][0]         = 0.0;
//         generalizedVelocity[iMode][1]         = 0.0;
//         generalizedVelocity[iMode][2]         = 0.0;
// 	}
// 
//     setGeneralizedVelocity(0,250.);
// 
// 	//initialize arrays
// 	modalForce     = new su2double[nModes];
//     modalForceLast = new su2double[nModes];
//     for (iMode=0; iMode < nModes; ++iMode) modalForce[iMode]        = 0.0;
//     for (iMode=0; iMode < nModes; ++iMode) modalForceLast[iMode]    = 0.0;
//     
//     /*--- compute mapping matrix ---*/
//     //calls to rbf functions
//     
//     // initialize RBF mapping functions
// //     CGeometry *RBFmapping = NULL;
// //     RBFmapping = new CRBFMapping(geometry, config);
// 
// };
// 
// void CSolver::solveStructure(CGeometry *geometry, CConfig *config){
//     unsigned short iMode, nModes;
//     su2double delta_q,delta_v;
//     su2double *Disps,*Vels,*dy;
//     su2double Omega,dt;
//     
// //     unsigned short rkflag = 0;
//     
// //     nModes = geometry->getnModes();
// // 
// //     dy= new su2double[8];//4th order RK scheme for state-space problem (nth_order x 2 size arrays)
// //     
// //     dt = 0.0005;    //TODO: this needs to come from the main unsteady time loop.
// //     
// //     // multiply mode tranpose by fluid force mapped to structure
// //     ComputeFluidModalForce(geometry);
// //     
// //     for( iMode = 0; iMode < nModes; ++iMode){
// //         delta_q  = 0.0;
// //         delta_v  = 0.0;
// //         
// //         Omega    = geometry->getFrequency(iMode);
// //         Vels     = getGeneralizedVelocity(iMode);
// //         Disps    = getGeneralizedDisplacement(iMode);
// // 
// //         dy[0] = Vels[1];                                        //rhs_u
// //         dy[1] = modalForceLast[iMode] - Omega*Omega*Disps[1];   //rhs_v
// // 
// //         dy[2] = Vels[2];                                        //rhs_u1
// //         dy[3] = modalForce[iMode] - Omega*Omega*Disps[2];       //rhs_v1
// // 
// //         delta_q = Disps[1] + 0.5*dt*(dy[2] + dy[0]);
// //         delta_v = Vels[1] + 0.5*dt*(dy[3] + dy[1]);
// // 
// //         setGeneralizedVelocity(iMode,delta_v);
// //         setGeneralizedDisplacement(iMode,delta_q);
// // 
// //     }
// 
//     //TODO: convert modal displacements to cartesian coordinates
//     //TODO: map structural displacements to CFD surface
//     //TODO: propagate surface mesh displacement to volume mesh
//     return;
// };
// 
// void CSolver::ComputeFluidModalForce(CGeometry *geometry) {
// 	unsigned short nModes,iMode,iDim;
//     unsigned long iPoint,nCSD_Points;
//     su2double f;
//         
// //     nModes      = geometry->getnModes();
// //     nCSD_Points = geometry->getnCSD_Points();
// //     
// //     f = 0.;
// //     
// // 	//project fluid force onto structural mode shapes
// // 	for( iMode = 0; iMode < nModes; ++iMode){
// //         // store value at previous iteration
// //         modalForceLast[iMode]   = modalForce[iMode];
// //         modalForce[iMode]       = 0.01;
// // 
// //         // compute new value
// //         for( iDim = 0; iDim < nDim; ++iDim){
// //             for( iPoint = 0; iPoint < nCSD_Points; ++iPoint) {
// //                 modalForce[iMode] += geometry->getModeVector(iMode,iDim,iPoint)*f;
// // //                 csdForce[iPoint + iDim*nCSD_Points];
// //             }
// //         }
// // 	}
// }
// 
// void CSolver::setGeneralizedDisplacement(unsigned short iMode, su2double val_disp) {
// 	generalizedDisplacement[iMode][0] = generalizedDisplacement[iMode][1];
// 	generalizedDisplacement[iMode][1] = generalizedDisplacement[iMode][2];
// 	generalizedDisplacement[iMode][2] = val_disp;
// };
// 
// void CSolver::setGeneralizedVelocity(unsigned short iMode, su2double val_disp) {
// 	generalizedVelocity[iMode][0] = generalizedVelocity[iMode][1];
// 	generalizedVelocity[iMode][1] = generalizedVelocity[iMode][2];
// 	generalizedVelocity[iMode][2] = val_disp;
// };
// 
// 
// su2double* CSolver::getGeneralizedDisplacement(unsigned short iMode) {
// 
//     static su2double sol_disp[3];
// 	sol_disp[0] 	= generalizedDisplacement[iMode][0];
// 	sol_disp[1] 	= generalizedDisplacement[iMode][1];
// 	sol_disp[2] 	= generalizedDisplacement[iMode][2];
//     
//     return sol_disp;
// }
// 
// su2double* CSolver::getGeneralizedVelocity(unsigned short iMode) {
// 
//     static su2double sol_vel[3];
// 	sol_vel[0] 	= generalizedVelocity[iMode][0];
// 	sol_vel[1] 	= generalizedVelocity[iMode][1];
// 	sol_vel[2] 	= generalizedVelocity[iMode][2];
// 
//     return sol_vel;
// }
// end of functions to solve modal equations of motion.
// ====================================================
